...
function avv_buildRiskTooltip(
  ...
  const derived =
    typeof complianceOverall === "number"
      ? `\nHinweis: (falls nicht direkt geliefert) Risiko ≈ 100 − Compliance → ${100 - complianceOverall}`
      : "";
...
function normalize(input: any) {
  ...
  const extrasMap = {
    internationale_übermittlungen:
      extrasSrc?.internationale_übermittlungen ?? extrasSrc?.international_transfers,
  ...
  // Risiko
  const riskOverall = isNum(input?.risiko_score?.gesamt)
...
export default function Page() {
  ...
  // Heuristik: manchmal ist risk_score.overall eigentlich der Compliance-Score
  ...
  const compliance = providedCompliance ?? (isLikelyComplianceScore ? agentOverall : complianceMatrix);
  const risk = isNum(raw?.risk_score?.overall)
    ? (raw!.risk_score!.overall as number)
    : compliance != null
    ? Math.max(0, 100 - (compliance as number))
    : null;
          ...
          {!data ? (
            ...
            <div className="table-responsive">
              <table className="table align-middle">
                ...
                <tbody>
                  {["internationale_übermittlungen", "haftungsbegrenzung", "gerichtsstand_recht"].map(
                    (k) => {
                      const label =
